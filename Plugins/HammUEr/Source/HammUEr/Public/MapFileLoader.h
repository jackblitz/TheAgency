...
class MapFileLoader{
	...
	public:
		...
		static void Set(MapSettingType inType, float value)
		{
			if (myLoader != NULL)
				...
				case MapSettingType::IntersectRange:
					MAPSettings::intersectRange = 1.0f / pow(10.f, (float)FMath::CeilToInt(FMath::LogX(10.f, FMath::Max(value, 1.0f))));
					break;
		...
		static void Set(MapSettingType inType, bool value)
		{
			if (myLoader != NULL)
				...
				case MapSettingType::Verbose:
					break;
				case MapSettingType::IgnoreQ1Rotation:
					MAPSettings::ignoreQ1Rotation = value;
					break;
				case MapSettingType::UseOriginalBuildMode:
					MAPSettings::originalBrushBuildMode = value;
					break;
				case MapSettingType::UseGroupBSP:
					MAPSettings::useGroupBSP = value;
					break;
				case MapSettingType::UseGroupEarClipping:
					MAPSettings::useEarClipper = value;
					break;
				case MapSettingType::UseUVRoundingMode:
					MAPSettings::roundUVs = value;
					break;
				}
		}
		...
		static EMapType GetMapType()
		{
			if (myLoader != NULL)
				return myLoader->GetMapType();

			return EMapType::Unknown;
		}

	private:
		static UStaticMesh* ImportBrush(MapBrush* workBrush, TArray<UMaterialInterface*> newMaterials, FVector3f & meshPosition, bool overrideDecouple = false, bool buildActor = true, bool noComplex = false);
		static bool RemovePreviousActorIfExists(FString name, bool doDestroy = false, FString tag = "", bool ignoreTag = true);
		...
		static void AddUserTagToActor(AActor* actor);
