...
void RecenterMesh(FRawMesh& rawMesh, FVector3f& newPosition, bool shiftOverride = false) // can't pass a default as a reference, of course, so
{
	if (!shiftOverride)
	{
...
void BuildRawMesh(std::vector<VMFTri*> triangles, FRawMesh& rawMesh, TArray<int32>& materialReindex)
{
	...
	{
		rawMesh.VertexPositions[i] = FVector3f(keptVertices[i]);	// why would you do this, UE5 rewrite person
	}
	...
	for (int tri = 0; tri < triangles.size(); tri++)
	{
		...
		rawMesh.WedgeTexCoords[0].Add(MAPConverters::MAPtoF2f(triangles[tri]->uv1));
		rawMesh.WedgeTangentX.Add(FVector3f::ZeroVector);
		rawMesh.WedgeTangentY.Add(FVector3f::ZeroVector);
		rawMesh.WedgeTangentZ.Add(MAPConverters::MAPtoF3f(triangles[tri]->n1));
		...
		rawMesh.WedgeTexCoords[0].Add(MAPConverters::MAPtoF2f(triangles[tri]->uv3));
		rawMesh.WedgeTangentX.Add(FVector3f::ZeroVector);
		rawMesh.WedgeTangentY.Add(FVector3f::ZeroVector);
		rawMesh.WedgeTangentZ.Add(MAPConverters::MAPtoF3f(triangles[tri]->n3));
		...
		rawMesh.WedgeTexCoords[0].Add(MAPConverters::MAPtoF2f(triangles[tri]->uv2));
		rawMesh.WedgeTangentX.Add(FVector3f::ZeroVector);
		rawMesh.WedgeTangentY.Add(FVector3f::ZeroVector);
		rawMesh.WedgeTangentZ.Add(MAPConverters::MAPtoF3f(triangles[tri]->n2));
	...
	for (TObjectIterator<AActor> Itr; Itr; ++Itr)
	{
		...
		if (toCheck == name)
		{
			if (Itr->Tags.Contains(tagToCheck))
			{
				...
				{
					if (HammUErSettings::toDeleteStaticMeshActorList.Contains(name))
						HammUErSettings::toDeleteStaticMeshActorList.Remove(name);
				}
				return true;
			}
		}
...
bool MapFileLoader::RemovePreviousActorIfExists(FString name, bool doDestroy, FString tag, bool ignoreTag)
{
	bool result = false;
	for (TObjectIterator<AActor> Itr; Itr; ++Itr)
	{
...
void MapFileLoader::AddUserTagToActor(AActor* actor)
{
	if (actor && HammUErSettings::AddUserTag)
		actor->Tags.AddUnique(FName(HammUErSettings::WantedTag));
}
...
void MapFileLoader::DoDummy(int32 entityIndex)
{
	...
	FRotator rotatorToUse = FRotator(UHammUErDeveloperSettings::Get()->ImportRotationValue.Y, UHammUErDeveloperSettings::Get()->ImportRotationValue.Z, UHammUErDeveloperSettings::Get()->ImportRotationValue.X);
	...
	if (UHammUErDeveloperSettings::Get()->AlwaysPrependFilename) 
	...
	RemovePreviousActorIfExists(tempName/*data->name*/, true, tag, false);

	FAS.Name = FName(*tempName);
	FAS.NameMode = FActorSpawnParameters::ESpawnActorNameMode::Requested;
	...
	if (UHammUErDeveloperSettings::Get()->ImportNonZero)
		propPosition += UHammUErDeveloperSettings::Get()->ImportOffset;

	if ((UHammUErDeveloperSettings::Get()->ImportRotation) && (UHammUErDeveloperSettings::Get()->DecoupleMeshesFromZero))
		propPosition = rotatorToUse.RotateVector(propPosition);

	if (UHammUErDeveloperSettings::Get()->ImportRotation)
		tempActor->SetActorRotation(rotatorToUse);

	tempActor->AddActorLocalRotation(FRotator(-data->angles.X, -data->angles.Y, data->angles.Z));
	...
	AddUserTagToActor(tempActor);

	// store our original data in the note
	std::map<std::string, std::string> originalData = data->GetOriginalData();
	std::string output = "";
	for (std::map<std::string, std::string>::iterator i = originalData.begin(); i != originalData.end(); i++)
		output += i->first + " = " + i->second + "\n";
	tempActor->Text = output.c_str();
...
void MapFileLoader::DoFuncBrush(int32 entityIndex, TArray<UMaterialInterface*> newMaterials)
{
	...
	RemovePreviousActorIfExists(tempName, true, tag, false);
	...
	FVector3f meshPosition = FVector3f(propPosition);//FVector::ZeroVector;
	...
	if (BPtoUse)  // we want to use the blueprint version
	{
		...
		if (data->modelname != "")
		{
			...
			if (UHammUErDeveloperSettings::Get()->PropScaleType == EHammUErPropScaleAdjust::Type::UEScale)
		...
		else
		{
			...
			float offsetCheck = FMath::Abs((FVector(meshPosition) - propPosition).Size());
			if (offsetCheck > 200.f) // objective vector offset thing to make sure we don't shift stuff too far away
				propPosition += FVector(meshPosition);
			else
				propPosition = FVector(meshPosition);
		}
		if (myMesh == NULL)
		...
		if (HammUErSettings::toDeleteStaticMeshActorList.Num() > 0)
		{
			if (HammUErSettings::toDeleteStaticMeshActorList.Contains(tempName))
				HammUErSettings::toDeleteStaticMeshActorList.Remove(tempName);
		}
		...
		tempActor->SetActorLocation(FVector(propPosition));
		USMC->SetStaticMesh(myMesh);
		if ((UHammUErDeveloperSettings::Get()->TwoSidedCull) && (data->myBrush->containsNoDraw || data->myBrush->containsDisplacements))
		...
		for (int i = 0; i < data->myOutputs.size(); i++)
		{
			...
			newOutput.fireTimes = data->myOutputs[i]->fireTimes;
	...
	else
	{
		...
		if (data->modelname != "")
		{
			...
			if (UHammUErDeveloperSettings::Get()->GenerateOverlaps)
...
void MapFileLoader::DoFuncBox(int32 entityIndex, TArray<UMaterialInterface*> newMaterials)
{
	...
	FVector3f meshPosition = FVector3f(propPosition);
...
void MapFileLoader::DoPropStatic(int32 entityIndex, MAPPropStatic* instancedProp)
{
	...
	if (data->modelname != "error")
	{
		...
		if (UHammUErDeveloperSettings::Get()->PlaceUnknownModels || (toSpawn != NULL))
		{
			if (toSpawn == NULL)
...
void MapFileLoader::DoLight(int32 entityIndex, MAPLight* instancedLight)
{
	...
	newPointLight->SetBrightness(data->intensity * (data->useIntensity ? UHammUErDeveloperSettings::Get()->LightFactor : 2));

	if (UHammUErDeveloperSettings::Get()->ImportNonZero)
		lightLocation += UHammUErDeveloperSettings::Get()->ImportOffset;
	...
	AddUserTagToActor(newPointLight);	
...
AHammuerPathNode* MapFileLoader::DoPath(int32 entityIndex)
{
	...
	if (UHammUErDeveloperSettings::Get()->ImportNonZero)
		pathNodeLocation += UHammUErDeveloperSettings::Get()->ImportOffset;
...
void MapFileLoader::DoSpotLight(int32 entityIndex, MAPSpotLight* instancedSpotLight)
{
	...
	newSpotLight->SetBrightness(data->intensity * (data->useIntensity ? UHammUErDeveloperSettings::Get()->LightFactor : 2));
	...
	AddUserTagToActor(newSpotLight);
...
void MapFileLoader::DoEnvironmentLight(int32 entityIndex, MAPEnvironmentLight* instancedEnvironmentLight)
{
	...
	AddUserTagToActor(newEnviroLight);	
	...
	if (HammUErSettings::InstanceInDatabase(data->instance_name.c_str()))
	{
		...
		for (int i = 0; i < numBrushes; i++)
		{
			...
			if (instancedMesh != NULL)
			{
				...
				bool foundOne = RemovePreviousActorIfExists(data->name.c_str(), true, tag);
				...
				if (UHammUErDeveloperSettings::Get()->ImportNonZero)
					propBase += UHammUErDeveloperSettings::Get()->ImportOffset;
...
void MapFileLoader::DoOverlay(int32 entityIndex, MAPOverlay* inputData)
{
	...
	if (UHammUErDeveloperSettings::Get()->ImportNonZero)
		decalLocation += UHammUErDeveloperSettings::Get()->ImportOffset;
...
#else
	while ((multiplier < 10) && (!Hit.HitObjectHandle.IsValid()))
#endif
	{
#if ENGINE_MAJOR_VERSION < 5
...
UStaticMesh* MapFileLoader::ImportBrush(MapBrush* workBrush, TArray<UMaterialInterface*> newMaterials, FVector3f &meshPosition, bool overrideDecouple, bool buildActor, bool noComplex)
{
	UWorld* editorWorld = GEditor->GetEditorWorldContext().World();
	FRotator rotatorToUse = FRotator(UHammUErDeveloperSettings::Get()->ImportRotationValue.Y, UHammUErDeveloperSettings::Get()->ImportRotationValue.Z, UHammUErDeveloperSettings::Get()->ImportRotationValue.X);
	...
	if (UHammUErDeveloperSettings::Get()->DecoupleMeshesFromZero || overrideDecouple)
		RecenterMesh(rawMesh, meshPosition, (meshPosition != FVector3f::ZeroVector));
	...
	if (UPackage::SavePackage(Package, NewAsset, *PackageFileName, SaveArgs))
	{
		...
		FAssetRegistryModule::AssetCreated(NewAsset);
		if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Warning, TEXT("Saving %s to package successful"), *NewPackageName);
		if (buildActor)
		{
			if (UHammUErDeveloperSettings::Get()->AlwaysPrependFilename)
			...
			AStaticMeshActor* tempActor = NULL;

			if (HammUErSettings::toDeleteStaticMeshActorList.Num() > 0)
			{
				if (HammUErSettings::toDeleteStaticMeshActorList.Contains(MeshName))
				{
					HammUErSettings::toDeleteStaticMeshActorList.Remove(MeshName);
					// don't respawn already existing actors
					tempActor = HammUErSettings::existingStaticMeshActorMap[MeshName];
				}
			}
			...
			if (!tempActor)
			{
				...
				tempActor = editorWorld->SpawnActor<AStaticMeshActor>(FAS);
				tempActor->SetActorLabel(MeshName);
				// _sometimes_ UE doesn't fucking clean up deleted actors, but thinks they're still alive even after garbage collection?! fuck it
				if (tempActor->GetFName() != FName(*MeshName))
				{
					UE_LOG(LogTemp, Error, TEXT("Warning! Couldn't create %s due to zombie actor(?!), renamed to %s"), *MeshName, *(tempActor->GetFName().ToString()));
					if (GEngine)
						GEngine->AddOnScreenDebugMessage(-1, 9001, FColor::Red, FString::Printf(TEXT("Warning! Couldn't create %s due to zombie actor(?!), renamed to %s"), *MeshName, *(tempActor->GetFName().ToString())));
				}
			}

			// thank you for this fucking change, whoever decided to do this in UE5
			FVector meshPos = FVector(meshPosition);

			if (UHammUErDeveloperSettings::Get()->ImportNonZero)
				meshPos += UHammUErDeveloperSettings::Get()->ImportOffset;

			if ((UHammUErDeveloperSettings::Get()->ImportRotation) && (UHammUErDeveloperSettings::Get()->DecoupleMeshesFromZero))
				meshPos = rotatorToUse.RotateVector(meshPos);

			if (UHammUErDeveloperSettings::Get()->ImportRotation)
				tempActor->SetActorRotation(rotatorToUse);

			tempActor->SetActorLocation(meshPos);
			...
			if (checkPos != meshPos)
				tempActor->SetActorLocation(meshPos);
			if ((UHammUErDeveloperSettings::Get()->TwoSidedCull) && (workBrush->containsNoDraw || workBrush->containsDisplacements))
				((UPrimitiveComponent*)tempActor->GetRootComponent())->bCastShadowAsTwoSided = true;
			tempActor->PostEditChange();
		}
		else
		...
		if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Error, TEXT("Saving %s to package failed"), *NewPackageName);
	return NULL;
}
...
void MapFileLoader::ImportIntoWorld(TArray<UMaterialInterface*> newMaterials, bool doAll)
{
	...
	if (FPaths::GetBaseFilename(cacheFilename).StartsWith(TEXT("HI_"), ESearchCase::CaseSensitive))
	{
		...
		NMIDPackage = CreatePackage(*NMIDPackageName);
		...
		GEditor->SavePackage(NMIDPackage, newWorld, *NMIDPackageFileName, SaveArgs);
	...
	SlowTask.MakeDialog();
	FVector3f meshPosition = FVector3f::ZeroVector;
	...
	for (int i = 0; i < numBrushes; i++)
	{
		...
		meshPosition = FVector3f::ZeroVector;
		if (workBrush)
		{
...
UStaticMesh* MapFileLoader::GenerateTemporaryModel(FString modelLocation, bool generateExclamation)
{
	...
	{
		newTri[i]->v1 *= UHammUErDeveloperSettings::Get()->ScaleUnknownModels;
		newTri[i]->v2 *= UHammUErDeveloperSettings::Get()->ScaleUnknownModels;
		newTri[i]->v3 *= UHammUErDeveloperSettings::Get()->ScaleUnknownModels;
	}

	FVector3f nowhere = FVector3f::ZeroVector;
	...
	if (debugPropMaterial == NULL)
	{
		...
		UPackage* matPackage = CreatePackage(*NewPackageName);
		...
		SaveArgs.SaveFlags = SAVE_None;
		if (UPackage::SavePackage(matPackage, newMaterial, *PackageFileName, SaveArgs))
		{
			matPackage->PostEditChange();
			FAssetRegistryModule::AssetCreated(matPackage);
		}
