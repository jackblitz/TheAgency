							...
							[
								...
								[
									...
									[
										...
										.ToolTipText(LOCTEXT("ImportHeader_tooltip", "Sets the import target root directory.\nOpening a file will add that filename as a directory under it, and make that the target.\nChoosing something *after* opening the file will set that directory as the target."))
									]
									+ SHorizontalBox::Slot()
									.AutoWidth()
									[
										SNew(SButton)
										.OnClicked(this, &SHammerDialogWidget::ResetImportHeader)
										.IsEnabled(true)
										.HAlign(HAlign_Center)
										.VAlign(VAlign_Center)
										.Text(LOCTEXT("ResetImportHeaderText", "X"))
										.ToolTipText(LOCTEXT("ResetImportHeader_tooltip", "Resets the import target root directory to empty.\nDoing this *after* opening the file will cause meshes to be put in a Meshes directory in the content root."))
									]
								]
							]
							+ SHorizontalBox::Slot()
...
FReply SHammerDialogWidget::HandleChooseFileButtonClicked()
{
	...
	materialList.Empty();
	const UHammUErDeveloperSettings* currentUserSettings = UHammUErDeveloperSettings::Get();

	//if (!currentUserSettings->RememberDirectory)
...
void SHammerDialogWidget::ReadMaterialFile()
{
	...
	{
		if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled) UE_LOG(LogTemp, Log, TEXT("Found a VMF specific material file, reading..."));
		...
		for (int i = 0; i < materialLines.Num(); i++)
		{
			...
			if (masterIndex != -1)
			{
				if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("+ Material already exists in master list. Is it different?"));
				if ((HammUErSettings::masterMaterialList[masterIndex]->newMaterial == NULL &&newmatname == ""))
				{
					...
					if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Yes, old one was empty, removed."));
				}
				else
					if ((HammUErSettings::masterMaterialList[masterIndex]->newMaterial != NULL && newmatname != "" && 
						...
						((tempint!=UHammUErDeveloperSettings::Get()->DefaultTextureSize.X&&tempint2!=UHammUErDeveloperSettings::Get()->DefaultTextureSize.Y)&&
					    ...
					    ))
					{
						...
						if (HammUErSettings::masterMaterialList[masterIndex]->newMaterial != NULL && newmatname == "")
						{
							...
							if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Yes, but it is blank, so we're keeping the old one."));
							continue;
						}
						else
						{
							materialList.Add(HammUErSettings::masterMaterialList[masterIndex]);
							materialListIndex.Add(masterIndex);
							if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("No."));
							continue;
						}
			}
			...
			if (newmatname == "")
			{
				...
				for (int j = 0; j < possiblesList.Num(); j++)
				{
					if ((HammUErSettings::AssetData[possiblesList[j]].AssetName.ToString().ToUpper() == tempTest.ToUpper()) || (HammUErSettings::AssetData[possiblesList[j]].AssetName.ToString().ToUpper() == (tempTest + "_mat").ToUpper()))
					{
						...
						if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Found a direct match in the asset database for unspecified material %s"), (*tempTest));
						break;
					}
				}
				...
				{
					index = 0;
					if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("No direct match in the asset database for unspecified material, picking close candidate %s"), *HammUErSettings::AssetData[possiblesList[index]].AssetName.ToString());
				}
				if (index > -1)
				{
			...
			else
			{
				...
				for (int j = 0; j < possiblesList.Num(); j++)
				{
					...
					{
							index = possiblesList[j];
							if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Found a direct match in the asset database for specified material %s"), (*newmatname));
							break;
					}
				}
				if (possiblesList.Num() > 0 && index == -1)  // pick the first one
				{
					index = 0;
					if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("No direct match in the asset database for specified material, picking close candidate %s"), *HammUErSettings::AssetData[possiblesList[index]].AssetName.ToString());
				}
		...
		for (int i = 0; i < oldMaterialList.Num(); i++)  // did anything change or get added?
		{
			if (HammUErSettings::FindInMaster(oldMaterialList[i]->oldName)==-1)//!HammUErSettings::masterMaterialList.ContainsByPredicate([&](TSharedPtr<FMaterialListEntry> other) {return other->oldName == oldMaterialList[i]->oldName; }))
			{
				...
				rewrite = true;
				if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Found a material in the VMF not in the material list or master list. Adding..."));
			}
			if (!materialList.ContainsByPredicate([&](TSharedPtr<FMaterialListEntry> other) {return other->oldName == oldMaterialList[i]->oldName; }))
			{
				...
				materialListIndex.Add(-1);
				rewrite = true;
				if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Found a material in the VMF not in the material list but in the master list. Adding..."));
			}
...
/*	UWorld* EditorWorld = GEditor->GetEditorWorldContext().World();
	...
	if (VMFileName != "")
	{
		if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Reloading master material list..."));
		HammUErSettings::LoadConfig(true);

		const UHammUErDeveloperSettings* currentUserSettings = UHammUErDeveloperSettings::Get();

		MapFileLoader::Open(VMFileName);
		MapFileLoader::Set(MapSettingType::ConversionRate, currentUserSettings->LocalConversionRate);

		MapFileLoader::Set(MapSettingType::IgnoreQ1Rotation, currentUserSettings->IgnoreQ1Rotation);

		MapFileLoader::Set(MapSettingType::RoundPoints, currentUserSettings->RoundPoints);
		MapFileLoader::Set(MapSettingType::RoundMeshPoints, currentUserSettings->RoundMeshPoints);
		MapFileLoader::Set(MapSettingType::RoundDisplacementPoints, currentUserSettings->RoundDisplacementPoints);

		MapFileLoader::Set(MapSettingType::DiscardErrorRange, currentUserSettings->DiscardErrorRange);
		MapFileLoader::Set(MapSettingType::IntersectRange, currentUserSettings->IntersectRange);
		MapFileLoader::Set(MapSettingType::SmoothWeldRange, currentUserSettings->SmoothWeldRange);

		MapFileLoader::Set(MapSettingType::SmoothingAngle, currentUserSettings->SmoothingAngle);
		MapFileLoader::Set(MapSettingType::DontSmooth, currentUserSettings->DoNotUseSmoother);
		MapFileLoader::Set(MapSettingType::DispAlphaChannel, (float)currentUserSettings->DefaultDisplacementChannel);
		MapFileLoader::Set(MapSettingType::PatchDivH, (float)currentUserSettings->PatchSubDiv.X);
		MapFileLoader::Set(MapSettingType::PatchDivV, (float)currentUserSettings->PatchSubDiv.Y);

		MapFileLoader::Set(MapSettingType::UseOriginalBuildMode, currentUserSettings->UseOriginalBuildMode);
		MapFileLoader::Set(MapSettingType::UseUVRoundingMode, currentUserSettings->UseUVRoundingMode);
		MapFileLoader::Set(MapSettingType::UseGroupBSP, currentUserSettings->UseGroupBSP);
		MapFileLoader::Set(MapSettingType::UseGroupEarClipping, currentUserSettings->UseGroupEarClipping);
		...
		EMapType LoadedMapType = MapFileLoader::GetMapType();
		if (LoadedMapType == EMapType::Valve220 && !currentUserSettings->IgnoreQ1Rotation)
		{
			EAppReturnType::Type result = FMessageDialog::Open(EAppMsgType::YesNo, FText::FromString("Opened what seems to be a Valve 220 format MAP file.\nDo you want to zero out the Quake 1 texture rotation information for this run?\n(This setting fixes bad texture alignments, and can be set globally in the HammUEr settings)"));
			if (result == EAppReturnType::Yes)
			{
				MapFileLoader::Set(MapSettingType::IgnoreQ1Rotation, true);
				// we need to rerun the first phase, because texture data for nodes gets read in the first pass together with the materials
				MapFileLoader::PhaseOne();
			}
		}

		TArray<FString> inList = MapFileLoader::GetMaterials();
		materialList.Empty();
		if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Checking master material list for matches..."));
		TSharedPtr<FMaterialListEntry> newEntry;
		...
		for (int i = 0; i < inList.Num(); i++)
		{
			...
			if (masterIndex != -1)
			{
				if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("Found %s"), *inList[i]);
			...
			{
				if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Warning, TEXT("Did not find %s in the master list, checking asset database"), *inList[i]);
				newEntry = MakeShareable(new FMaterialListEntry());
				...
				if (newEntry->newMaterial)
				{
					...
					if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Log, TEXT("+ found a material for %s"), *inList[i]);
				}
				else
				{
					newEntry->textureWidth = UHammUErDeveloperSettings::Get()->DefaultTextureSize.X;
					newEntry->textureHeight = UHammUErDeveloperSettings::Get()->DefaultTextureSize.Y;
					newEntry->newMaterialName = "";
					if (UHammUErDeveloperSettings::Get()->VerboseDebugEnabled)UE_LOG(LogTemp, Error, TEXT("- did not find a material for %s"), *inList[i]);
				}
				newEntry->ignore = false;
				if (newEntry->oldName.Contains("nodraw") || newEntry->oldName.ToLower() == "clip" || newEntry->oldName.ToLower() == "origin" || newEntry->oldName.Contains("toolsorigin") || newEntry->oldName.Contains("toolsclip"))
					newEntry->ignore = true;
...
FReply SHammerDialogWidget::RunConvert()
{
	...
	FString oldName;
	HammUErSettings::toDeleteStaticMeshActorList.Empty();
	HammUErSettings::existingStaticMeshActorMap.Empty();
	...
	if (myWorld != NULL)
	{
		for (TActorIterator<AStaticMeshActor> ActorIterator(myWorld); ActorIterator; ++ActorIterator)
		{
			...
			{
				FString name = ActorIterator->GetName();
				HammUErSettings::toDeleteStaticMeshActorList.AddUnique(name);
				HammUErSettings::existingStaticMeshActorMap.Add(name, *ActorIterator);
			}
		}
	}
	else
	...
	for (int i = 0; i < materialList.Num(); i++)
	{
		...
		if (materialList[i]->newMaterial)
		{
			if (materialList[i]->textureWidth<1)
				textureWidths.Add(UHammUErDeveloperSettings::Get()->DefaultTextureSize.X);
			else
				textureWidths.Add(materialList[i]->textureWidth);
			if (materialList[i]->textureHeight<1)
				textureHeights.Add(UHammUErDeveloperSettings::Get()->DefaultTextureSize.Y);
			else
	...
	if (UHammUErDeveloperSettings::Get()->ReimportCleanup && HammUErSettings::toDeleteStaticMeshActorList.Num() > 0)
	{
		TArray<FString> deleted;
		for (int i = 0; i < HammUErSettings::toDeleteStaticMeshActorList.Num(); i++)
		{
			if (HammUErSettings::existingStaticMeshActorMap.Contains(HammUErSettings::toDeleteStaticMeshActorList[i]))
			{
				AStaticMeshActor* work = HammUErSettings::existingStaticMeshActorMap[HammUErSettings::toDeleteStaticMeshActorList[i]];
				if (work != NULL)
				{
					work->K2_DestroyActor();
					deleted.Add(HammUErSettings::toDeleteStaticMeshActorList[i]);
				}
			}
		}
		for (int i = 0; i < deleted.Num(); i++)
			HammUErSettings::toDeleteStaticMeshActorList.Remove(deleted[i]);

		if (HammUErSettings::toDeleteStaticMeshActorList.Num() > 0)
		{
			UE_LOG(LogTemp, Warning, TEXT("Problem deleting extraneous brushes?"));
			for (int i = 0; i < HammUErSettings::toDeleteStaticMeshActorList.Num(); i++)
				UE_LOG(LogTemp, Warning, TEXT("%i %s"), i, *HammUErSettings::toDeleteStaticMeshActorList[i]);
			HammUErSettings::toDeleteStaticMeshActorList.Empty();
		}
	}
