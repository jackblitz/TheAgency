...
{
	buttonPushed = true;
	TSharedPtr<SDockTab> previousActive = FGlobalTabmanager::Get()->TryInvokeTab(HammUErTabName);
	
	ExtraTabs();
	
	FGlobalTabmanager::Get()->TryInvokeTab(HammUErTabName); // switch back to main tab
}
