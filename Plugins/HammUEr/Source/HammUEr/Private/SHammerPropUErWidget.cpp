...
{
	return FIntPoint((int)FMath::Pow(2.f, (float)FMath::CeilLogTwo(inPoint.X)), (int)FMath::Pow(2.f, (float)FMath::CeilLogTwo(inPoint.Y)));
}
...
void SHammerPropUErWidget::BuildRawMesh(std::vector<VMFTri*> triangles, FRawMesh& rawMesh, TArray<int32>& materialReindex)
{
	...
	{											// actually, we don't, I just ran into a shitty bug
		if (loadTris[i])
		{
			triVertexIndex.Add(keptVertices.AddUnique(MAPConverters::MAPtoF3f(triangles[i]->v1)));
			triVertexIndex.Add(keptVertices.AddUnique(MAPConverters::MAPtoF3f(triangles[i]->v2)));
			triVertexIndex.Add(keptVertices.AddUnique(MAPConverters::MAPtoF3f(triangles[i]->v3)));
		}
	}
		...
		if (modelList[currentImportFile]->doImport)
		{
			...
			if (modelLoader != NULL)
			{
				if (modelLoader->Open(TCHAR_TO_UTF8(*modelList[currentImportFile]->name)))
				{
					...
					if (triangles.size() > 0)
					{
						if (UHammUErDeveloperSettings::Get()->PropScaleType == EHammUErPropScaleAdjust::Type::InPropUEr)
						{
							float convertUE = 100.0f / currentUserSettings->LocalConversionRate;
